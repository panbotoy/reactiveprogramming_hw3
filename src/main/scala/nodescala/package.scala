...
package object nodescala {
        ...
        case x::xs =>
          x.onComplete {
            case Success(success) =>
              all(xs).onComplete {
                case Success(xsSuccess) =>
                  p.success(success::xsSuccess)
                case Failure(xsFailure) =>
                  p.failure(xsFailure)
              }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      val p = Promise[S]()
      f.onComplete { tryValue => p.complete(Try(cont(f))) }
      p.future
    }
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      val p = Promise[S]()
      f.onComplete { tryValue => p.complete(Try(cont(tryValue))) }
      p.future
    }
